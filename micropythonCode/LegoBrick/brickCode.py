...
direction, l_stick_ver, r_stick_hor, r_stick_ver, l_trigger, r_trigger, setting1, setting2 = [0]*8
buttons = [0]*8
button_last = False
qt_button = 0
...
def on_rx(control):
    global qt_button, button_last, direction, l_stick_ver, r_stick_hor, r_stick_ver, l_trigger, r_trigger, setting1, setting2, buttons
    direction, l_stick_ver, r_stick_hor, r_stick_ver, l_trigger, r_trigger, setting1, setting2, buttons_char = struct.unpack("bbbbBBiiB", control)
    for i in range(8):
        if buttons_char & 1 << i:
            buttons[i]=1
        else:
            buttons[i]=0
    if not button_last and buttons[0]:
        qt_button += 1
    button_last = buttons[0]
...
while True:
    if receiver.is_connected():
        if qt_button % 2 == 1:
            motor_pair.start(direction, speed=60)
        else:
            motor_pair.stop()
    else:
        motor_pair.stop()
